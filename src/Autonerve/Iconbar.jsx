...
function Iconbar() {
  ...
  const onDragStart = (event, shape) => {
    // remove fileObject (not serializable)
    const { fileObject, ...safeShape } = shape;
    const nodeData = JSON.stringify(safeShape);
    event.dataTransfer.setData('application/reactflow', nodeData);

    // store real file globally if exists
    if (fileObject) {
      window.__draggedFile__ = fileObject;
    }
  ...
  const handleFileChange = (e) => {
    ...
    const newShape = {
      ...
      fileObject: isPDF ? file : file, // ✅ keep the File here
    };
