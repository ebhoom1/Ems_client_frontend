...
export default function KpiCards({ summary }) {
  ...
  useEffect(() => {
    console.log("KpiCards received summary:", summary); 
  ...
  const getCardBackgroundColor = (title, percentage) => {
    if (title === "Diesel Percentage") {
      const fuelPercent = parseFloat(percentage) || 0;
      if (fuelPercent <= 20) return "#9d1623ff"; 
      if (fuelPercent < 50) return "#b45c13ff"; 
      return "#21983dff"; 
    }
    return "#236a80"; 
  };
...
export default function KpiCards({ summary }) {
  ...
  useEffect(() => {
    if (summary.fuelPercent <= 20 && summary.fuelPercent > 0)
      setShowLowFuelAlert(true);
    else setShowLowFuelAlert(false);
  }, [summary.fuelPercent]);

  const getFuelIcon = (p) => {
    const v = parseFloat(p) || 0;
    if (v > 80) return <FaBatteryFull size={28} className="text-light" />;
    if (v > 50) return <FaBatteryThreeQuarters size={28} className="text-light" />;
    if (v > 20) return <FaBatteryHalf size={28} className="text-light" />;
    if (v > 5) return <FaBatteryQuarter size={28} className="text-light" />;
    return <FaBatteryEmpty size={28} className="text-light" />;
  };
  ...
  const getBg = (t, p) => {
    if (t === "Diesel Percentage") {
      const v = parseFloat(p);
      if (v <= 20) return "#9d1623ff";
      if (v < 50) return "#b45c13ff";
      return "#21983dff";
    }
    return "#236a80";
  };
    ...
    <>
      ...
      <Row className="g-4 mb-4 mt-3">
        {cards.map((item, idx) => (
          <Col key={idx} xs={12} md={6} lg={3}>
            <div
              style={{ backgroundColor: getBg(item.title, summary.fuelPercent) }}
              className="shadow-sm border-0 rounded-3 h-75"
            >
              <Card.Body className="d-flex flex-column justify-content-center p-4">
                <div className="d-flex align-items-center">
                  {item.icon}
                  <Card.Title className="text-center text-light small mb-0 ms-2">
                    {item.title}
                  </Card.Title>
                </div>
                <h2 className="fw-bold text-start ms-4 fs-4 text-light mb-3">
                  {item.value}
                </h2>
              </Card.Body>
            </div>
