...
useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        setFormData({
            userName: '',
            userType: 'user',
            stations: [{ stationName: '', stationType: '', kg: '', date: moment().format('YYYY-MM-DD') }],
        });
        setEditing(false);
        setEditId(null);
        fetchWasteData();
  ...
  const handleEdit = (waste) => {
    ...
    if (userType === 'admin' || timeDiff <= 24) {
      const editableFormData = {
        ...waste,
        stations: waste.stations.map((station) => ({
          ...station,
          date: moment(station.date).format('YYYY-MM-DD'),
        })),
      };
      ...
      Swal.fire('Access Denied', 'You can only edit data within 24 hours. Please contact an admin.', 'warning');
    }
  };
  ...
  {wasteData.length > 0 ? (
    wasteData
      .filter((waste) => waste.stations.some(station => station.stationType === 'generator'))
      .filter((waste) => {
        if (userType === 'user') {
      ...
      .length > 0 ? (
            ...
            <div key={waste._id} className="col-md-4 mb-4">
              <div className="card border shadow">
                <div className="card-body">
                  ...
                  {waste.stations
                    .filter(station => station.stationType === 'generator')
                    .map((station) => (
        ...
        <p className="text-center" style={{ color: 'red' }}>No data available for the selected user. Please add !!</p>
      )
  ) : (
    <p className="text-center" style={{ color: 'red' }}>No generator data available.</p>
  )}
</div>
          ...
          >
            <Modal.Header closeButton>
              <Modal.Title>{editing ? 'Edit Generator Details' : 'Add Generator Details'}</Modal.Title>
            </Modal.Header>
            <Modal.Body>
