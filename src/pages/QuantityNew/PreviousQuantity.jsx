...
export default function PreviousQuantity() {
  ...
  const { userType, userData } = useSelector((s) => s.user);
  ...
  // ------------------------------------------------------------------
  // ✅ Determine the effective user name (with CONTI → EGL1 mapping)
  ...
  const effectiveUserName =
    ...
    userParam === "CONTI"
      ? "EGL1"
      : storedUserId || currentUserName || userParam;

  console.log("🔹 PreviousQuantity using username:", effectiveUserName);
  ...
  // ✅ Fetch + filter data
  useEffect(() => {
    if (!effectiveUserName || !month || !year) return;
    setLoading(true);

    axios
      .get(`${API_URL}/api/difference/month/${effectiveUserName}/${month}/${year}`)
      .then((res) => {
        ...
        // ✅ EGL1-specific correction
        if (effectiveUserName === "EGL1") {
          filtered = filtered.filter(
            (d) =>
              d.stackName !== "STP intlet" &&
              d.stackName !== "STP iutlet"
      ...
      .catch((err) => console.error("❌ Error fetching PreviousQuantity:", err))
      .finally(() => setLoading(false));
  }, [effectiveUserName, month, year]);
  ...
  useEffect(() => {
    ...
    const uniqStacks = Array.from(new Set(flatData.map((d) => d.stackName))).sort(
      (a, b) => a.localeCompare(b)
    );
    setStackNames(uniqStacks);

    const uniqDates = Array.from(new Set(flatData.map((d) => d.date))).sort(
      (a, b) =>
  ...
  const handleDownloadCSV = () => {
    const header = ["Date", ...stackNames].join(",");
    const csvRows = rows.map((r) =>
      [r.date, ...stackNames.map((s) => (r[s] != null ? r[s].toFixed(2) : ""))].join(",")
    );
    const csv = [header, ...csvRows].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `${effectiveUserName}_flow_diff_${month}_${year}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };
  ...
  const handleDownloadPDF = () => {
    const doc = new jsPDF({ orientation: "landscape" });
    doc.text(`Flow Differences for ${effectiveUserName} — ${month}/${year}`, 14, 16);
    doc.autoTable({
      startY: 22,
      head: [["Date", ...stackNames]],
      body: rows.map((r) => [
        r.date,
        ...stackNames.map((s) => (r[s] != null ? r[s].toFixed(2) : "")),
      ]),
      styles: { fontSize: 8 },
      headStyles: { fillColor: [33, 150, 243] },
    });
    doc.save(`${effectiveUserName}_flow_diff_${month}_${year}.pdf`);
  };
  ...
  return (
    <div className="container-fluid px-0 py-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-3 px-4">
        <h1 className="h4 m-0">
          {`Previous Quantity for ${effectiveUserName} — ${month}/${year}`}
        </h1>
        <div>
          <button
            onClick={handleDownloadCSV}
