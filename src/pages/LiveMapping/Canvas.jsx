...
function Canvas() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (saveResponse.ok) {
        alert('Live Station created successfully');
  ...
  const fetchMapData = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/get-live-station?userName=${userName}`);
    ...
    <div className="react-flow-container">
      ...
      {mapData ? (
        <div>
          ...
          />
          <button onClick={() => setMapData(null)} className="btn btn-primary mt-2">
            Edit
          </button>
        </div>
      ) : (
