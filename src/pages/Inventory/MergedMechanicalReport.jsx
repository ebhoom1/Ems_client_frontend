...
useEffect(() => {
  ...
  useEffect(() => {
    if (!adminType) return;
    axios.get(`${API_URL}/api/logo/${encodeURIComponent(adminType)}`)
      .then(r => {
...
<h6 className='text-center'>Merged Report of  {companyName && ` — ${companyName}`}&nbsp;— Reports for{' '}
  ...
  {new Date(year, month - 1).toLocaleString('default', { month: 'long' })} {year}
        ...
        {filteredReports.length > 0 ? filteredReports.map((report, idx, arr) => {
            ...
            <div key={report._id} style={{
             ...
             /*  pageBreakAfter: idx < arr.length - 1 ? 'always' : 'auto' */
              ...
              <div style={{ marginBottom: 8, fontSize: 12 }}>  
                ...
                {report.siteName && <> &nbsp;|&nbsp; <strong>Site:</strong> {report.siteName}</>}
                {report.capacity && <> &nbsp;|&nbsp; <strong>Capacity:</strong> {report.capacity}</>}
              </div>
<div
  style={{
    marginBottom: 8,
...
>
  ...
  {report.isWorking === 'yes' ? 'Yes' : 'No'}
              ...
              <table style={{ width: '100%', border: '1px solid #000', borderCollapse: 'collapse', marginBottom: 12 }}>
                <tbody>
                  <tr>
                    ...
                    <td style={td}>{report.technician?.name} — {report.technician?.designation}</td>
                  </tr>
                  <tr>
              ...
              <div style={{ fontSize: '13px', margin: '4px 0', display: 'flex', alignItems: 'center' }}>
                  ...
                  {report.photos && report.photos.length > 0
                    ? report.photos.map((url, i) => (
                        <img
                          ...
                          crossOrigin="anonymous"
                          alt={`Photo ${i + 1}`} 
                          style={{ maxWidth: 120, maxHeight: 120, border: '1px solid #ddd' }}
                ...
                <table style={{ width: '100%', border: '1px solid #000', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>
                      ...
                      {report.columns.map((col,i) => <th key={i} style={th}>{col}</th>)}
                  ...
                  <tbody>
                    {report.entries.map((e,i) => (
                      <tr key={`${report._id}-${e.id}`}>
                        ...
                        {e.checks.map((c,j) => {
                          const val = c.value === 'ok' ? '✓' : c.value === 'fail' ? '✕' : '—';
                          const style = { ...tdC, color: c.value === 'ok' ? 'green' : c.value === 'fail' ? 'red' : undefined };
                          return <td key={j} style={style}>{val}</td>;
                        })}
                        <td style={{ ...td, color: e.checks.some(c=>c.value==='fail') ? 'red' : e.checks.some(c=>c.value==='ok') ? 'green' : undefined }}>
                          {e.remarks || '—'}
                        </td>
                      </tr>
                    ))}
          ...
          <p>No mechanical reports found for the selected user, month, and year.</p>
        )}
      </div>
    </div>
  );
