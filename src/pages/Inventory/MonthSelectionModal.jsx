...
export default function MonthSelectionModal({ onClose, reportType, onMonthSelected }) {
  ...
  const [year, setYear] = useState(new Date().getFullYear());
  const [month, setMonth] = useState(new Date().getMonth() + 1);
  const [checklistType, setChecklistType] = useState("SBR"); // default SBR
  const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = data.users || [];
        setUsers(list);
        if (list.length) setSelectedUser(list[0].userName);
      } catch (err) {
        console.error(err);
        toast.error("Error fetching user list");
      }
  ...
  const handleSubmit = (e) => {
    ...
    // Engineer / Safety reports handled manually
    if (reportType === "engineer" || reportType === "safety") {
      onMonthSelected(year, month, selectedUser, checklistType);
      return;
    }

    // Electrical / Mechanical direct redirect
    if (reportType === "electrical") {
      navigate(`/report/electrical/download/${selectedUser}/${year}/${month}`);
    } else if (reportType === "mechanical") {
      navigate(`/mechanical-report/${selectedUser}/${year}/${month}`);
        ...
        <div className="modal-content">
          ...
          <div className="modal-body">
            <form onSubmit={handleSubmit}>
              ...
              {/* ✅ Checklist dropdown only for SAFETY reports */}
              {reportType === "safety" && (
                <div className="mb-3">
                  <label htmlFor="checklistType" className="form-label">
                    Checklist Type
                  </label>
                  <select
                    id="checklistType"
                    ...
                    required
                  >
                    <option value="SBR">SBR Checklist</option>
                    <option value="ASP">ASP Checklist</option>
                    <option value="MBR">MBR Checklist</option>
                  </select>
                </div>
              )}
