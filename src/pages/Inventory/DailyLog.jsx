...
export default function DailyLog() {
    ...
    const handleAddLog = async () => {
      ...
      // 2) Time entries
      const mainRows = Array.from(root.querySelectorAll("table.main-log tbody tr"));
                  ...
                  remarksCell = (
                    <td rowSpan={6} className="p-0">
                      ...
                      <table className="table table-sm table-bordered mb-0 treated-water text-center">
                        ...
                        <thead><tr><th colSpan="2">TREATED WATER</th></tr></thead>
                        <tbody>
                          {treatedWaterParams.map(p=>(
                            <tr key={p}>
                              <td className="text-start">{p}</td>
                              <td>
                    ...
                    {equipmentList.map((eq, i)=>(
                      <React.Fragment key={eq}>
                        <td className="text-center">
                          <input type="radio" name={`onoff-${t}-${i}`} value="on" className="form-check-input m-0" disabled={!isUser}/>
                        </td>
                        <td className="text-center">
                          <input type="radio" name={`onoff-${t}-${i}`} value="off" className="form-check-input m-0" disabled={!isUser}/>
                        </td>
                      </React.Fragment>
                    ))}
                    {remarksCell}
