...
const MonthlyPh = () => {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.post(`${API_URL}/api/monthly-report`, payload);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
        ...
        const tableHead = [["Date", "MLSS (mg/ltr)", "pH - TREATED WATER", "Comment"]];
        ...
        if (isAdmin) {
            ...
            if (mlssChartRef.current) {
                ...
                const mlssCanvas = await html2canvas(mlssChartRef.current, { scale: 2 });
        ...
        // 5. Save PDF
        doc.save(`${targetUser.siteName}_${monthNames[month]}_${year}_Report.pdf`);
  ...
  const handleDownloadCSV = () => {
    ...
    readings.forEach(reading => {
      ...
      csvContent += `${dateStr},${reading.mlss || ''},${reading.ph || ''},${comment}\n`;
    });
  ...
  const downloadPdfButtonStyle = { // Renamed
    ...buttonStyle,
    backgroundColor: '#e74c3c', // Red for PDF
    borderColor: '#e74c3c',
  };

  const downloadCsvButtonStyle = { // New
    ...buttonStyle,
            ...
            <div className="col-12">
              ...
              {/* {(isAdmin || isOperator) && (
                <div style={{
                  ...
                  Logged in as: <strong>{currentUser?.userName}</strong> (Type: {currentUser?.userType})<br/>
                  Header Selected: <strong>{selectedUserId || "NONE"}</strong><br/>
                  Final Target User: <strong>{targetUser.userName || "NULL"}</strong> (ID: {targetUser.userId || "NULL"})
                </div>
              )}
              */}
              ...
              {!targetUser.userName ? (
                ...
                <div style={cardStyle}>
                  <div style={promptStyle}>
                    <i className="fas fa-hand-pointer" style={{ fontSize: '3rem', marginBottom: '1.5rem', color: '#236a80' }}></i>
                    <h3 style={{fontWeight: '600', color: '#236a80'}}>Please Select a User</h3>
                    <p style={{fontSize: '1.1rem', color: '#34495e', maxWidth: '400px'}}>
                ...
                <> 
                  ...
                  <div style={cardStyle}>
                    <div className="row">
                      {/* Data Table */}
                      <div className={isAdmin ? "col-lg-6" : "col-12"}> {/* Changed to lg-6 */}
                        <div style={{
                                ...
                                <tr key={index} style={{
                                  ...
                                  <td style={{padding: '8px 10px'}}>
                                    <input
                                      ...
                                      value={reading.mlss}
                                      onChange={(e) => handleInputChange(index, "mlss", e.target.value)}
                                      disabled={(!isOperator && !isAdmin) || loading}
                                      style={inputStyle}
                                    />
                                  </td>
                                  <td style={{padding: '8px 10px'}}>
                      ...
                      {isAdmin && (
                        <div className="col-lg-6"> {/* Changed to lg-6 */}
                          <div style={{
                            border: '3px dotted #3498db',
                            borderRadius: '12px',
                            padding: '20px',
